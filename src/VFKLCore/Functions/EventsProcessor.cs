...
namespace VFKLCore.Functions
{
    ...
    public class EventsProcessor
    {
                ...
                if (data.ContentType == "application/xml")
                {
                    ...
                    if (invitation != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(invitation.MottakerEposter))
                        {
                            ...
                            // Building the invitation that is to be sent to the recipients
                            StringBuilder invitation_body = new StringBuilder();

                            invitation_body.Append("Hei<br><br>");
                            invitation_body.Append($"{invitation.Navn} har invitert deg til å delta i en vurdering av et læremiddel.<br><br>");
                            invitation_body.Append($"Læremiddelet skal vurderes for bruk iht. følgende læreplan: {invitation.Læreplan} ({invitation.LæreplanKode})<br>");
                            invitation_body.Append($"Læremiddelet som skal vurderes er: {invitation.Læremiddel}<br><br>");
                            invitation_body.Append($"For å vurdere læremiddelet brukes tjenesten «Veileder for vurdering av læremidler i {assessmenttypeString}» fra Utdanningsdirektoratet. Når du følger lenken for å starte din vurdering blir du bedt om å logge inn på din Feide konto for å bruke tjenesten.<br><br>");

                            DateTime frist;
                            if (DateTime.TryParse(invitation.VurderingsFrist, out frist))
                            {
                                invitation_body.Append($"Frist for vurderingen er: {frist.ToString("D", new CultureInfo("no"))}<br><br>");
                            }
                            else
                            {
                                invitation_body.Append($"Frist for vurderingen er: {invitation.VurderingsFrist} <br><br>");
                            }
                            ...
                            StringBuilder confirmation_body = new StringBuilder();
                            StringBuilder invitedEmails = new StringBuilder();
                            ...
                            confirmation_body.Append($"<div style=\"border: 2px solid #93EDBE; padding: 10px\"><b>Invitasjoner er blitt sendt til følgende eposter:</b> <br/>{invitedEmailsTrimmed} </div><br/><br/>");
                            confirmation_body.Append("Under linjen er det lagt ved en kopi av innholdet til invitasjonene som er sendt ut til denne gruppevurderingen. Informasjon som læremiddel, læreplan og frist er synlig for mottakerne her. <br/><br/>");
                            confirmation_body.Append("Du kan ikke svare på denne eposten. Dersom du opplever tekniske utfordringer, kontakt vfklsupport@udir.no <br/><br/>");
                            confirmation_body.Append("Lykke til med vurderingen!<br/>");
                            confirmation_body.Append("<hr><br/>");
                            confirmation_body.Append(invitation_body);

                            SendGridClient client = new SendGridClient(_settings.SendGridApiKey);

                            try
                            {
                                SendGridMessage msg = MailHelper.CreateSingleEmailToMultipleRecipients(from, recipients, $"Vurdering av læremiddelet {invitation.Læremiddel} (no-reply)", null, invitation_body.ToString());
                            ...
                            {
                                _logger.LogError("Error sending invitation email: " + ex.Message);
                            }

                            if (!string.IsNullOrEmpty(invitation.BrukerEpost))
                            {
                                try
                                {
                                    EmailAddress confirmation_recipient = new EmailAddress(invitation.BrukerEpost);
                                    SendGridMessage msg = MailHelper.CreateSingleEmail(from, confirmation_recipient, $"Bekreftelse på utsendt gruppevurdering av {invitation.Læremiddel} (no-reply)", null, confirmation_body.ToString());
                                    Response response = await client.SendEmailAsync(msg).ConfigureAwait(false);
                                    if (response != null && !response.IsSuccessStatusCode)
                                    {
                                        _logger.LogError(response.StatusCode.ToString());
        ...
        private async Task SaveGroupInvitation(GruppeInvitasjon invitation)
        {
            ...
            var culture = GetCultureInfo("no");
            var cultureInfo = culture.TextInfo;
            invitation.LæremiddelLeverandør = cultureInfo.ToTitleCase(invitation.LæremiddelLeverandør.Trim());
        ...
        private async Task SaveAssessment(Vurdering vurdering)
        {
            ...
            var culture = GetCultureInfo("no");
            var cultureInfo = culture.TextInfo;
            vurdering.LæremiddelLeverandør = cultureInfo.ToTitleCase(vurdering.LæremiddelLeverandør.Trim());
